...
if ( ! function_exists( 'bloghub_posted_on' ) ) :
	...
	function bloghub_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'bloghub' ),
...
if ( ! function_exists( 'bloghub_posted_by' ) ) :
	...
	function bloghub_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'bloghub' ),
	...
	function bloghub_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'bloghub' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'bloghub' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'bloghub' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'bloghub' ),
						array(
							'span' => array(
								'class' => array(),
							),
