...
/*-----------------------------------------------------------------------------------*/
/*  Bloghub Customizer Controls
/*-----------------------------------------------------------------------------------*/

class Bloghub_Misc_Control extends WP_Customize_Control 
    {
...
class Bloghub_Textarea_Custom_Control extends WP_Customize_Control
    {
        public function render_content() 
        {
            ?>
...
class Bloghub_Theme_Support extends WP_Customize_Control 
    {
        public function render_content() 
        {
            echo wp_kses_post( 'Upgrade to <a href="#">bloghub Plus</a> to be able to change the section order and styling!', 'saint' );
        }
    }

if ( ! function_exists( 'bloghub_sanitize_checkbox' ) ) 
    {
        function bloghub_sanitize_checkbox( $input ) 
        {
            if ( $input == 1 ) 
            {
                return 1;
 ...
 */
function bloghub_sanitize_repeatable_data_field( $input , $setting ){
            ...
            if ( isset( $fields[ $id ] ) ){
                switch( strtolower( $fields[ $id ]['type'] ) ) {
                    ...
                    case 'color':
                        ...
                        break;
                    case 'coloralpha':
                        $data[ $i ][ $id ] = bloghub_sanitize_color_alpha( $value );
                        break;
                    case 'checkbox':
                        $data[ $i ][ $id ] =  bloghub_sanitize_checkbox( $value );
                        break;
                    case 'select':
                        $data[ $i ][ $id ] = '';
...
class Bloghub_Editor_Custom_Control extends WP_Customize_Control
{
    /**
 ...
 */
class Bloghub_Alpha_Color_Control extends WP_Customize_Control {
 ...
 * @access public
 */
class Bloghub_Customize_Repeatable_Control extends WP_Customize_Control {
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if ( ! isset( $args['allow_unlimited'] ) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters( 'bloghub_reepeatable_max_item', absint( $args['max_item'] ) );
        }  else {
            $this->max_item = absint( $args['max_item'] );
        }

        $this->changeable =  isset(  $args['changeable'] ) && $args['changeable'] == 'no' ? 'no' : 'yes';
        $this->default_empty_title =  isset(  $args['default_empty_title'] ) && $args['default_empty_title'] != '' ? $args['default_empty_title'] : esc_html__( 'Item', 'bloghub' );
    ...
    public function render_content() {
        ...
        <div class="repeatable-actions">
            <span class="button-secondary add-new-repeat-item"><?php  esc_html_e( 'Add an item', 'bloghub' ); ?></span>
        </div>
        <?php
    }
            ...
            <div class="widget">
                <div class="widget-top">
                    ...
                    <div class="widget-title">
                        <h4 class="live-title"><?php  esc_html_e( 'Item', 'bloghub' ); ?></h4>
                    </div>
                </div>
                                ...
                                <# if ( field.type ){ #>
                                    ...
                                    #>
                                        ...
                                        <# } else if ( field.type == 'media' ) { #>
                                            ...
                                            <div class="actions">
                                                <button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php  esc_html_e( 'Remove', 'bloghub' ) ?></button>
                                                <button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'bloghub' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'bloghub' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php  esc_html_e( 'Add', 'bloghub' ); ?> <# } else { #> <?php  esc_html_e( 'Change', 'bloghub' ); ?> <# } #> </button>
                                        ...
                                        <# }  else if ( field.type == 'icon'  ) { #>
                                            ...
                                            <a href="#" class="remove-icon"><?php esc_html_e( 'Remove', 'bloghub' ); ?></a>
                                        <# }  #>
                            ...
                            <div class="widget-control-actions">
                                <div class="alignleft">
                                    <span class="remove-btn-wrapper">
                                        <a href="#" class="repeat-control-remove" title=""><?php  esc_html_e( 'Remove', 'bloghub' ); ?></a> |
                                    </span>
                                    <a href="#" class="repeat-control-close"><?php  esc_html_e( 'Close', 'bloghub' ); ?></a>
                                </div>
                                <br class="clear">
                            </div>
...
function bloghub_enqueue_editor(){
...
function bloghub_customizer_control_scripts(){
    wp_enqueue_media();
    wp_enqueue_script( 'jquery-ui-sortable' );
    wp_enqueue_script( 'wp-color-picker' );
    wp_enqueue_style( 'wp-color-picker' );
    wp_enqueue_script( 'bloghub-customizer', get_template_directory_uri() . '/inc/admin/customizer_js.js', array( 'customize-controls', 'wp-color-picker' ) );
}

add_action( 'customize_controls_enqueue_scripts', 'bloghub_customizer_control_scripts', 99 );
add_action( 'customize_controls_enqueue_scripts', array( 'Bloghub_Editor_Scripts', 'enqueue' ), 95 );
